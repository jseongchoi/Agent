...
정리되어 있다.
최신 에이전트 기능 기준 TODO와 이번 구현 내역은 [`LATEST_AGENT_TODO.md`](LATEST_AGENT_TODO.md)에
따로 정리되어 있다.
...
- API auth: optional bearer token for `/api/*`
- API structured errors: stable `error.code/category/retryable`
...
- eval: `python -m semicon_agent.eval`
...
에이전트 동작 회귀 검증은 eval suite로 확인한다.

```powershell
python -m semicon_agent.eval
```

editable install 후에는 다음 console script도 사용할 수 있다.

```powershell
semicon-agent-eval
```

eval은 yield, SPC, report 요청을 deterministic `MockLLM`으로 실행하고 핵심 문구와 tool 오류 여부를 확인한다.
...
| `GET /api/runs/{run_id}/otel` | span-like observability export |
...
| Structured errors | `semicon_agent/core/errors.py` |
| Trace events | `semicon_agent/core/trace.py` |
| Observability export | `semicon_agent/core/observability.py` |
...
| Eval suite | `semicon_agent/eval.py` |
| Tests | `tests/` |
